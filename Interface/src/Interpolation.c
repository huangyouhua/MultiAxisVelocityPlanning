...
Matrix_t Interpolation(int len, int Degree, int Fre, Matrix_t* joint_angle, Matrix_t* K, double* synthetic_velocity_lookahead, double* max_synthetic_acceleration, double* max_synthetic_velocity, double* Length)
{
	...
	double* Velocity_syn = (double*)malloc(sizeof(double) * (NumOfInterPoint));
	Matrix_t Position = create_mat(NumOfInterPoint, Degree);
	Matrix_t Velocity = create_mat(NumOfInterPoint, Degree);
	...
	for (row = 0; row < len - 1; row++)
	{
		if (row == 0)
		{
			for (count = 0; count < (int)(ta[row] * Fre); count++)
			{
				...
				{
					dt = time;
					delta_Length = pow(dt / ta[row], 2) * segment1[row];
					Position.data[t][column] = joint_angle->data[row][column] + P.data[row][column] * delta_Length / Length[row];
					Velocity.data[t][column] = (synthetic_velocity_lookahead[row] + (sampling_period * count) / ta[row] * (actual_max_velocity[row] - synthetic_velocity_lookahead[row])) * K->data[row][column];
				}
				Velocity_syn[t] = (synthetic_velocity_lookahead[row] + (sampling_period * count) / ta[row] * (actual_max_velocity[row] - synthetic_velocity_lookahead[row]));
				t++;
			}
			...
			for (count = 0; count < (int)((ta[row] + tl[row] - past_time) * Fre); count++)
			{
				...
				{
					dt = time - ta[row];
					delta_Length = segment1[row] + dt * segment2[row] / tl[row];
					Position.data[t][column] = joint_angle->data[row][column] + P.data[row][column] * delta_Length / Length[row];
					Velocity.data[t][column] = actual_max_velocity[row] * K->data[row][column];
				}
				Velocity_syn[t] = actual_max_velocity[row];
				t++;
			}
			past_time = time;
			...
			for (count = 0; count < (int)((ta[row] + td[row] + tl[row] - past_time) * Fre); count++)
			{
				...
				for (column = 0; column < Degree; column++)
				{
					delta_Length = segment1[row] + segment2[row] + 0.5 * dt * (2 * actual_max_velocity[row] * td[row] - dt * (actual_max_velocity[row] - synthetic_velocity_lookahead[row + 1])) / td[row];
					Position.data[t][column] = joint_angle->data[row][column] + P.data[row][column] * delta_Length / Length[row];
					Velocity.data[t][column] = (actual_max_velocity[row] - dt / td[row] * (actual_max_velocity[row] - synthetic_velocity_lookahead[row + 1])) * K->data[row][column];
				}
				Velocity_syn[t] = (actual_max_velocity[row] - dt / td[row] * (actual_max_velocity[row] - synthetic_velocity_lookahead[row + 1]));
				t++;
			}
		}
		else
		{
			...
			for (count = 0; count < (int)((my_sum(ta, row) + my_sum(tl, row - 1) + my_sum(td, row - 1) - past_time) * Fre); count++)
			{
				...
				for (column = 0; column < Degree; column++)
				{
					...
					delta_Length = 0.5 * dt * (2 * synthetic_velocity_lookahead[row] * ta[row] + (actual_max_velocity[row] - synthetic_velocity_lookahead[row]) * dt) / ta[row];
					Position.data[t][column] = joint_angle->data[row][column] + P.data[row][column] * delta_Length / Length[row];
					Velocity.data[t][column] = (synthetic_velocity_lookahead[row] + dt / ta[row] * (actual_max_velocity[row] - synthetic_velocity_lookahead[row])) * K->data[row][column];
				}
				Velocity_syn[t] = (synthetic_velocity_lookahead[row] + dt / ta[row] * (actual_max_velocity[row] - synthetic_velocity_lookahead[row]));
				t++;
			}
			...
			for (count = 0; count < (int)((my_sum(ta, row) + my_sum(tl, row) + my_sum(td, row - 1) - past_time) * Fre); count++)
			{
				...
				for (column = 0; column < Degree; column++)
				{
					...
					delta_Length = segment1[row] + dt / tl[row] * segment2[row];
					Position.data[t][column] = joint_angle->data[row][column] + P.data[row][column] * delta_Length / Length[row];
					Velocity.data[t][column] = actual_max_velocity[row] * K->data[row][column];
				}
				Velocity_syn[t] = actual_max_velocity[row];
	...
	free(td);							td = NULL;
	free(tl);							tl = NULL;
	free(Velocity_syn);					Velocity_syn = NULL;
	free_mat(&Velocity);
